...
9. **Sports digest** (opt-in via `PRIMARY_RECIPIENTS` + `SPORTS_LEAGUES` +
   SMTP2GO env, with `SPORTS_RECIPIENTS` as optional extra subscribers;
   no-ops when unconfigured). A daily, 7-days-a-week
