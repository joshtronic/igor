...
for t in date mkdir cp find basename rm; do
...
d="$(find "$STATE/crash-logs" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | head -1)/"
...
eq "no marker -> nothing preserved" "0" "$(find "$STATE2/crash-logs" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')"

# An empty worktree argument (unset PR_WORKTREE) -> safe no-op.
crashlog_preserve 1 "$TMP/s3" "" >/dev/null 2>&1
eq "empty worktree arg is a no-op" "0" "$(find "$TMP/s3/crash-logs" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')"
...
eq "keeps CRASHLOG_KEEP (20) newest" "20" "$(find "$PSTATE/crash-logs" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')"
...
d4="$(find "$STATE4/crash-logs" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | head -1)/"
...
eq "scratch: missing dir -> no-op"      "0" "$(find "$TMP/s5/crash-logs" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')"
