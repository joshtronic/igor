...
- `--max-turns` is hardcoded to 100 at every `claude_run_with_cost`
  call site (issue work, PR-review, maintenance triage, site-work --
  4 sites across `bin/tick.sh` + `bin/site-work-block.sh`). It was 50;
  thorough verification (Playwright screenshots + a full play-through)
  is turn-expensive, and 50 was capping runs mid-verification. This is
  an opinionated system: the cap is a constant, not an env knob. Bump
  the number in code if 100 turns out to be tight.
