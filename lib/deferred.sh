...
# check can false-positive (seen live on a real ticket), so it removes `Status/Blocked`
...
# auto gate-check can false-positive (seen live on a real ticket), so a machine MET
