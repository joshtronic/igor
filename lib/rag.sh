...
#       by $TICK_PID. Returns 0 on success (index ready) or
...
#   AGENT_HOME         path to the harness repo (for rag.py + venv)
#   AGENT_STATE_DIR    where state lives ($HOME/.local/state/agent by default)
#   AGENT_BRAIN_PATH   path to the brain clone (defaults computed below)
#   TICK_PID     pid of the parent tick.sh process. tick.sh
...
: "${AGENT_HOME:?AGENT_HOME must be set before sourcing lib/rag.sh}"
: "${AGENT_STATE_DIR:=$HOME/.local/state/agent}"

_RAG_VENV="$AGENT_STATE_DIR/rag-venv"
_RAG_MARKER_DIR="$AGENT_STATE_DIR/rag-built"

_rag_brain_path() {
  printf '%s' "${AGENT_BRAIN_PATH:-$AGENT_STATE_DIR/repos/igor/brain}"
}

_rag_marker_path() {
  printf '%s/%s' "$_RAG_MARKER_DIR" "${TICK_PID:-$$}"
}
...
ensure_rag_built() {
  ...
  if ! AGENT_BRAIN_PATH="$brain" \
       "$_RAG_VENV/bin/python" "$AGENT_HOME/bin/rag.py" build; then
...
rag_query() {
  ...
  local brain
  brain=$(_rag_brain_path)
  AGENT_BRAIN_PATH="$brain" \
    "$_RAG_VENV/bin/python" "$AGENT_HOME/bin/rag.py" query \
      "$query" -k 5 2>/dev/null || true
}

# Cleanup helper -- tick.sh hooks this into its EXIT trap so the
# per-tick marker doesn't accumulate in $AGENT_STATE_DIR/rag-built/.
rag_cleanup_marker() {
